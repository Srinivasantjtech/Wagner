    ...
    public class TBWTemplateEngine
    {
              ...
              //  string attname = "";
                ...
                if (dsrecords.Tables[0].Rows.Count > 0)
                {
                  ...
                  //  objErrorHandler.createexecutiontmielog();
                    ...
                    if (HttpContext.Current.Request.QueryString["fid"] != null)
                    {
                        ...
                        if (tmpds != null && tmpds.Tables.Count > 0)
                        {
                           // objErrorHandler.CreateLog("inside product page" + "GET_PARENT_CATEGORY_ID_PATH_INPUT_FAMID" + tmpds.Tables[0].Rows.Count + "---" + HttpContext.Current.Session["EA"].ToString());
                            ...
                            try
                            {
                                string[] catpath = HttpContext.Current.Session["EA"].ToString().Replace( "AllProducts////WESAUSTRALASIA////" ,"").Split(new string[] { "////" }, StringSplitOptions.None);
                            ...
                            _stmpl_records.SetAttribute("TBT_CATEGORY_ID", tmpds.Tables[0].Rows[0]["CATEGORY_ID"]);
                             ...
                             if (checkcathpath != tmpds.Tables[0].Rows[0]["CATEGORY_PATH"].ToString())
                             {
                    ...
                    _stmpl_records.SetAttribute("TBT_COST", cdr["COST"].ToString());
                  ...
                  //  sw2.Start();
                    ...
                    if (_stmpl_records.GetAttribute("TBT_PARENT_FAMILY_ID") != null)
                    {
                       ...
                       // strurlnew = objHelperService.SimpleURL(_stmpl_records, "AllProducts////WESAUSTRALASIA////" + _stmpl_records.GetAttribute("TBT_PARENT_FAMILY_ID") + "////" + categorypath + "////" + cdr["FAMILY_name"].ToString().Replace("<sub1>", " ").Replace("</sub1>", "").Replace("<sub2>", " ").Replace("</sub2>", ""), "fl.aspx");
                ...
                //    _sPriceTable = GetProductPriceTable(Convert.ToInt32(cdr["PRODUCT_ID"]));
                    ...
                    if (HttpContext.Current.Session["USER_ID"] != null && HttpContext.Current.Session["USER_ID"] != "")
                        userid = HttpContext.Current.Session["USER_ID"].ToString();
                    CustomerType = objUserServices.GetCustomerType(Convert.ToInt32(userid));
                    //objErrorHandler.CreateLog(_Prod_Stock_Flag + "product_id" + cdr["STRING_VALUE"].ToString());
                    if (_Prod_Stock_Flag.ToLower() == "0")
                  ...
                  //  Stopwatch swpd = new Stopwatch();
                   // swpd.Start();
                    ...
                    if (HttpContext.Current.Session["PRO_CAT_NAME"] != null)
                        _stmpl_records.SetAttribute("TBT_CT_NAME", HttpContext.Current.Session["PRO_CAT_NAME"].ToString());
                    else
            ...
            try
            {
              ...
              //  GetSupplierDetail();
                ...
                else
                {
                    ...
                    _stmpl_container.SetAttribute("CART_AMOUNT", dsCart.Tables[0].Rows[0]["ORDER_AMOUNT"]);
                    //modified by:indu
                    ...
                    _stmpl_records1.SetAttribute("CART_AMOUNT", dsCart.Tables[0].Rows[0]["ORDER_AMOUNT"]);
        ...
        public string ST_Top_Cart_itemMobile(DataSet dsCart, string _Tbt_Order_Id)
        {
            ...
            try
            {
                ...
                //string _BCEAPath = GetEAPath();
                _Tbt_Ship_URL = "/orderDetails.aspx?bulkorder=1&amp;Pid=0&amp;ORDER_ID=" + _Tbt_Order_Id;
                _Tbt_chkout_URL = "/checkout.aspx?" + EncryptSP(_Tbt_Order_Id.ToString());
                    ...
                    foreach (DataRow dr in dsCart.Tables[0].Rows)//For Records
                    {
                       ...
                       // category = strsupplierName;
                        ...
                        if (tmpds != null && tmpds.Tables.Count > 0)
                        {
                            ...
                            //    , "pd.aspx", true);
                            newurl = objHelperServices.SimpleURL_Str("AllProducts////WESAUSTRALASIA////" + tmpds.Tables[0].Rows[0]["PARENT_FAMILY_ID"].ToString() + "////" + tmpds.Tables[0].Rows[0]["product_ID"].ToString() + "=" + dr["Code"] + "////" + "////" + (catpath.Length >= 1 ? catpath[0] : " ") + "////" + (catpath.Length >= 2 ? catpath[1] : " ") + "////" + tmpds.Tables[0].Rows[0]["FAMILY_name"].ToString(), "pd.aspx", true);
        ...
        public string ST_Top_Load()
        {
            ...
            try
            {
             ...
             //   }
               ...
               //     _stmpl_container.SetAttribute("Cache_Top", "Cache_Top_Replace");
                    ...
                    else if (Requesturlstring.Contains("ps.aspx") == true || Requesturlstring.Contains("pl.aspx") == true || Requesturlstring.Contains("bb.aspx") == true)
                    {
                        if (dssubcat != null && dssubcat.Tables[0].Rows.Count > 0)
                        {
                            ...
                            foreach (DataRow drsubcat in dssubcat.Tables[0].Rows)
                            {
                                ...
                                _stmpl_records.SetAttribute("TBW_ATTRIBUTE_INPUT", drsubcat[0].ToString().Replace("$", "").Replace(" ", ""));
  ...
  string[] pfid = sptcookieval[0].Split(',') ;
                ...
                foreach (DataRow dr in tmpds.Tables[0].Rows)//For Records
                {
                    ...
                    if (dr1 != null)
                    {
                        if (prevprodid != dr["PRODUCT_ID"].ToString())
                        {
                            ...
                            URL_RW_PATH = objHelperServices.SimpleURL_Str("AllProducts////WESAUSTRALASIA////" + dr["FAMILY_ID"] + "////" + dr["product_ID"] + "=" + dr["Code"] + "////" + "////" + (catpath.Length >= 1 ? catpath[0] : " ") + "////" + (catpath.Length >= 2 ? catpath[1] : " ") + "////" + dr["FAMILY_name"], "pd.aspx", true);

                            string desc = dr["DESCRIPTION"].ToString();

                            _stmpl_records.SetAttribute("TBT_REWRITEURL", URL_RW_PATH);
                            _stmpl_records.SetAttribute("TBT_TWEB_IMAGE1", GetImagePath(dr["TWEB Image1"]));
                            _stmpl_records.SetAttribute("TBT_FAMILY_NAME", dr["FAMILY_NAME"]);
                            // _stmpl_records.SetAttribute("TBT_FAMILY_TITLE", dr["FAMILY_NAME"].ToString().Replace('"', ' '));
                ...
                foreach (DataRow dr in tmpds.Tables[0].Rows)//For Records
                {
                    ...
                    _stmpl_records = _stg_records.GetInstanceOf(cellnewnp);
                           ...
                           string title = textInfo.ToTitleCase(dr["FAMILY_NAME"].ToString().ToLower());
                            _stmpl_records.SetAttribute("TBT_FAMILY_NAME", title);
                            // _stmpl_records.SetAttribute("TBT_FAMILY_TITLE", dr["FAMILY_NAME"].ToString().Replace('"', ' '));
                            // _stmpl_records.SetAttribute("TBT_DESCRIPTIONS", dr["DESCRIPTION"]);
         ...
         public string ST_NewProduct_Load()
        {
            ...
            try
            {
               ...
               // DataSet tmpds   = objHelperDB.GetDataSetDB("exec STP_TBWC_PICK_NEW_PRODUCT_WAG 16,0," + userid);

               // DataSet tmpds = objHelperDB.GetDataSetDB("exec STP_TBWC_PICK_NEW_PRODUCT_WAG_FAMPRO 16,0," + userid);
                DataSet tmpds = new DataSet();
               // JObject o1;
                tmpds = (DataSet)HttpContext.Current.Cache["Cache_NEWPRODUCT"];
            //    tmpds = EasyAsk.GetCategoryAndBrand_Applicationstart("New Products");
                if (tmpds != null && tmpds.Tables[0].Rows.Count > 0)
                {
               ...
               // bool isenable = IsEcomenabled();
                ...
                int ictrecords = 0;
                string URL_RW_PATH = string.Empty;
                string cellnewnp = "NewProduct\\cellnew";

                DataSet dsPrice = objHelperDB.GetProductPriceEA("", tmpds.Tables["AllProducts"].Rows[0][0].ToString(), userid,"New");
                foreach (DataRow dr in tmpds.Tables[0].Rows)//For Records
                {
                    _stmpl_records = _stg_records.GetInstanceOf(cellnewnp);
                 ...
                 // URL_RW_PATH=   objHelperServices.Cons_NewURlASPX("", "AllProducts////WESAUSTRALASIA////" + "////" + dr["CATEGORY_PATH"] + "////" + dr["FAMILY_ID"] + "=" + dr["FAMILY_name"] + "////" + dr["product_ID"] + "=" + dr["Code"], "pd.aspx", true, "");
                    ...
                    if (dr["PROD_COUNT"].ToString() == "1")
                    {
                        URL_RW_PATH = objHelperServices.SimpleURL_Str("AllProducts////WESAUSTRALASIA////" + dr["FAMILY_ID"] + "////" + dr["product_ID"] + "=" + dr["Code"] + "////" + "////" + (catpath.Length >= 1 ? catpath[0] : " ") + "////" + (catpath.Length >= 2 ? catpath[1] : " ") + "////" + dr["FAMILY_name"], "pd.aspx", true);
                       // _stmpl_records.SetAttribute("TBT_REWRITEURL_PAGE", "pd");
                      //  _stmpl_records.SetAttribute("TBT_CHECK_PAGE", true);
                    }
                    else
                    {
                      //  string newurl = string.Empty;
                        string categorypath = (catpath.Length >= 1 ? catpath[0] : " ") + "////" + (catpath.Length >= 2 ? catpath[1] : " ");
                       // newurl = "AllProducts////WESAUSTRALASIA////" + dr["FAMILY_ID"] + "////" + categorypath + "////" + dr["FAMILY_name"];
                       // objHelperServices.SimpleURL_Str("AllProducts////WESAUSTRALASIA////" + dr["FAMILY_ID"] + "////" + categorypath + "////" + dr["FAMILY_name"];
                        URL_RW_PATH =  objHelperServices.SimpleURL_Str("AllProducts////WESAUSTRALASIA////" + dr["FAMILY_ID"] + "////" + categorypath + "////" + dr["FAMILY_name"],"fl.aspx",true);
                       // objHelperServices.SimpleURL(_stmpl_records, newurl, "fl.aspx");
                       // URL_RW_PATH = objHelperServices.SimpleURL_Str("AllProducts////WESAUSTRALASIA////" + dr["FAMILY_ID"] + "////" + dr["product_ID"] + "=" + dr["Code"] + "////" + "////" + (catpath.Length >= 1 ? catpath[0] : " ") + "////" + (catpath.Length >= 2 ? catpath[1] : " ") + "////" + dr["FAMILY_name"], "pd.aspx", true);
                        //_stmpl_records.SetAttribute("TBT_REWRITEURL_PAGE", "fl");
                       // _stmpl_records.SetAttribute("TBT_CHECK_PAGE", false);
                    }
                 ...
                 //   DataSet tmpdsPrice = objHelperDB.GetProductPriceEA(dr["FAMILY_ID"].ToString(), dr["PRODUCT_ID"].ToString(), userid);

                    DataRow[] foundRows = dsPrice.Tables[0].Select("Product_id = '" + dr["Product_id"].ToString() + "'");

                    DataTable tmpdsPrice = foundRows.CopyToDataTable();
                    if (tmpdsPrice != null)
                    {
                        if (tmpdsPrice.Rows[0]["price"].ToString() != string.Empty)
                        {

                            //dr1["PRODUCT_PRICE"] = dr["price"].ToString();
                           // string tmpprice = string.Empty;
                           // tmpprice = objHelperServices.CheckPriceValueDecimal(tmpdsPrice.Rows[0]["price"].ToString());
                           // dr["COSt"] = tmpprice;
                            dr["COSt"] = objHelperServices.CheckPriceValueDecimal(tmpdsPrice.Rows[0]["price"].ToString());
                        }
                    }
                    ...
                    _stmpl_records.SetAttribute("TBT_YOURCOST", dr["MIN_PRICE"]);
                 ...
                 foreach (DataRow dr in dsrecords_dup.Tables[0].Rows)//For Records
                {
                    ...
                    DataRow[]  dr1 = dsrecords.Tables[0].Select("Category_id='" + dr["CATEGORY_ID"] + "' ");
                      ...
                      if (dr_cat_id_upper != WesNewsCategoryId && !(dr_cat_id_upper.Contains("SPF-"))&& dr1.Length >0 )
                      {
                          ...
                          _stmpl_records.SetAttribute("TBT_REWRITEURL", url);
                          _stmpl_records.SetAttribute("TBT_CATEGORY_NAME", dr1[0]["CATEGORY_NAME"]);
                          lstrecords[ictrecords] = new TBWDataList(_stmpl_records.ToString());
                          ictrecords++;
                      }
                 }
