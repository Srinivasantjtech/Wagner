                                ...
                                foreach (DataRow dr in Sqltb.Rows)
                                {
                                    foreach (DataRow dr1 in ds.Rows)
                                    {
                                        if (dr["PRODUCT_ID"].ToString().ToUpper() == dr1["PRODUCT_ID"].ToString().ToUpper())
                                        {
                                            ...
                                            //}
                                            /* DB Call Price*/
                                            objErrorhandler.CreateLog(dr1["STOCK_STATUS_DESC"].ToString() + dr1["PRODUCT_ID"].ToString());
                                            objErrorhandler.CreateLog( "Sustitute"+dr1["PROD_SUBSTITUTE"].ToString() +"---"+ dr1["PRODUCT_ID"].ToString());
                                            dr1["PROD_SUBSTITUTE"] = dr["PROD_SUBSTITUTE"].ToString();

                                            string url = HttpContext.Current.Request.Url.ToString().ToLower();

                                            if ((url.Contains("ps.aspx") == false) && (url.Contains("mps.aspx") == false))
                                            {
                                                try
                                                {
                                                    if (dr1["STOCK_STATUS_DESC"].ToString() == "DISCONTINUED NO LONGER AVAILABLE" && dr["PROD_SUBSTITUTE"].ToString() != "")
                                                    {
                                                        ProductServices objproductservice = new ProductServices();
                                                        objErrorhandler.CreateLog(dr1["PRODUCT_CODE"].ToString());

                                                        DataTable substituteproduct = objproductservice.GetSubstituteProduct(dr1["PRODUCT_CODE"].ToString());

                                                        if (substituteproduct.Rows.Count > 0)
                                                        {
                                                            objErrorhandler.CreateLog("sustututeproduct stock status:"+substituteproduct.Rows[0]["substitute_stockstatus"].ToString());
                                                            if (substituteproduct.Rows[0]["substitute_stockstatus"].ToString().Trim() == "DISCONTINUED NO LONGER AVAILABLE")
                                                            {
                                                                dr1["PROD_SUBSTITUTE"] = "";
...
/***/
            ...
            string url1 = HttpContext.Current.Request.Url.ToString().ToLower();

            objErrorhandler.CreateLog(url1+"--"+ds.Rows.Count.ToString() + "--easyask b4 remove");  
                    if ((url1.Contains("ps.aspx")==false ) && (url1.Contains("mps.aspx")==false ) )
                       
                    {

                        try
                        {
                            DataRow[] dr = ds.Select("STOCK_STATUS_DESC<>'DISCONTINUED NO LONGER AVAILABLE' OR PROD_SUBSTITUTE <>''");
                            if (dr.Length > 0)
                            {
                              
                                ds = dr.CopyToDataTable();
                                ds.TableName = "FamilyPro";
                            }
                        }
                        catch(Exception ex)
                    ...
                    objErrorhandler.CreateLog(ds.Rows.Count.ToString() + "--easyask after remove");  
                                ...
                                foreach (DataRow dr1 in ds.Rows)
                                {
                                    if (dr["PRODUCT_ID"].ToString().ToUpper() == dr1["PRODUCT_ID"].ToString().ToUpper())
                                    {
                                       ...
                                       // dr1["SORT_ORDER"] = dr["SORT_ORDER"];
                                        dr1["PROD_SUBSTITUTE"] = dr["PROD_SUBSTITUTE"].ToString().Trim();
//Added by Indu ..Not to show discontinued stock
                                        try
                                        {

                                            objErrorhandler.CreateLog("PROD_SUBSTITUTE " + dr["PROD_SUBSTITUTE"].ToString() + "STOCK_STATUS_DESC--" + dr1["STOCK_STATUS_DESC"].ToString());
                                            if (dr1["STOCK_STATUS_DESC"].ToString() == "DISCONTINUED NO LONGER AVAILABLE" )
                                              
                                            {
                                                if ((dr1["PROD_SUBSTITUTE"].ToString() != "" || dr1["PROD_SUBSTITUTE"].ToString() == null))
                                                {
                                                    ProductServices objproductservice = new ProductServices();
                                                    objErrorhandler.CreateLog("insite SubstituteProduct logic" + dr1["PRODUCT_CODE"].ToString());

                                                    DataTable substituteproduct = objproductservice.GetSubstituteProduct(dr1["PRODUCT_CODE"].ToString());

                                                    if (substituteproduct.Columns.Contains("substitute_stockstatus"))
                                                    {
                                                        objErrorhandler.CreateLog(substituteproduct.Rows[0]["substitute_stockstatus"].ToString());
                                                        if (substituteproduct.Rows[0]["substitute_stockstatus"].ToString().Trim() == "DISCONTINUED NO LONGER AVAILABLE")
                                                        {
                                                            dr1["PROD_SUBSTITUTE"] = "";
                                                            objErrorhandler.CreateLog("inside familypage substitute");
                                                ...
                                                {
                                                    objErrorhandler.CreateLog("inside else is not familypage substitute");
                                                    dr1["PROD_SUBSTITUTE"] = "";
                                                }
                                                ...
                                                //{
                                                //dr1["PROD_SUBSTITUTE"] = "";
                                                //objErrorhandler.CreateLog("inside else familypage substitute");
                                                //}
                                            }
                                        }
                                            catch(Exception ex)
                ...
                //Added by Indu ..Not to show discontinued stock
                objErrorhandler.CreateLog("Family page " + ds.Rows.Count.ToString() + "b4 check");
                try
                {
                    DataRow[] dr = ds.Select("STOCK_STATUS_DESC<>'DISCONTINUED NO LONGER AVAILABLE' OR PROD_SUBSTITUTE <>''");
                    if (dr.Length > 0)
                ...
                objErrorhandler.CreateLog("Family page " + ds.Rows.Count.ToString() + "after check");   
                AttributePro.Tables.Add(ds); 
            }
            catch (Exception ex)
            {
