        ...
        function DRPshippment() {
         ...
         //   alert("x");
            ...
            document.getElementById("ctl00_maincontent_loading").style.display = "block";
             enablePayNow();
             console.log(dropin);
            dropin.clearSelectedPaymentMethod();             // commented by smith
...
{
%> <div class="dropdown-menu" id="navcart-div"> <ul class="inner-cart" > <%=Cartdetail%> </ul> </div> <% } %>--%>
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.d == 1) {
                    ...
                    $("#<%=BtnL3Continue.ClientID%>").val("Submit Order");
                }
                showPrice();
                TopCartItems();
                return true;
...
function ProceedToPayment(obj) {
    $.ajax({
        ...
        success: function (response) {
           ...
           //     alert("smith");
                ...
                $("#<%=lblpaypaltotamt.ClientID%>").text($("#<%=totalAmount.ClientID%>").text());
                console.log("paypal " + $("#<%=lblpaypaltotamt.ClientID%>").text());
                    //           $("#<%=ttOrder.ClientID%>").focus();
                    $("#<%=PHOrderConfirm.ClientID%>").hide();
                    ScrollToControl("ctl00_maincontent_divl4payment");
                    if ($("#<%=totalAmount.ClientID%>").text() == "") {
                        var totamt = obj.orderInfo.TotalAmount;
                    }
                    else {
                        var totamt = $("#<%=totalAmount.ClientID%>").text();
                    }
 ...
 function noThanksBtnClick() {
    ...
    function cartItems() {
        ...
        function TopCartItems() {
        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: "Express_Checkout.aspx/TopCartItems",
            data: "{'str':'" + '' + "'}",
            datatype: "json",
            async: false,
            success: function (response) {
                ...
                var orderDetails = "";
                var topcart = "";
                 topcart += '<a data-toggle="dropdown" class="dropdown-toggle" href="" rel="nofollow" aria-expanded="true">';
                 topcart += ' Cart: ' + obj.CART_COUNT + ' items -  ' + "$ " + obj.CART_AMOUNT + '  <b class="glyphicon glyphicon-menu-down"></b></a>';
                
                 orderDetails += '<div class="dropdown-menu" id="navcart-div"> <ul class="inner-cart" >';
                $(obj.TopCartList).each(function (index, res) {
                    orderDetails += '<li class="padding_10 border_bottom padding_bottom15 padding_top15" style=" padding-bottom: 15px; padding-top: 15px;padding: 10px;border-bottom: thin solid #ddd;">';
                    orderDetails += '<div class="pull-left">';
                    orderDetails += '<img  src="' + res.imageSrc + '" class="pull-left" height="50px" width="50px"> </div>';
                    orderDetails += '<div class="pull-left margin_left15 drop_width">';
                    orderDetails += '<a href="' + res.TBT_REWRITEURL + '" style="text-align:left; text-decoration:none; color:#333; padding:0px; display:block;" rel="nofollow">';
                    orderDetails += '<div>  <strong>Order-Code : ' + res.CODE + ' </strong> </div> <div>' + res.FAMILY_NAME + '</div>';
                    orderDetails += '<div class="green_clr margin_top"> <strong>Price : ' + "$ " + res.COST + ' </strong> </div> </a>'
                    orderDetails += '</div> <div class="clear"></div> </li>';
                });
                 orderDetails += '<li class="padding_10 width_100 text-center green_bg btn_absolute ">';
                 orderDetails += '<a href="' + obj.VIEW_ORDER + '" style="color:#fff;" class="white_color font_weight" rel="nofollow">View Cart</a></li>';
                 orderDetails += '</ul> </div>';
                
                $("#navcart").append(topcart + orderDetails);
                return true;
            },
            error: function (err) {
                //          alert("error12");
        ...
        $.ajax({
            ...
            success: function (response) {
               ...
               $("#<%=totalAmount.ClientID%>").text(response.d.totalAmount);
                ...
                //console.log("paypal amount" + $("#<%=lblpaypaltotamt.ClientID%>").text());
...
//            });
    ...
    <script type="text/javascript">
        ...
        $("#<%=drpSM1.ClientID %>").on("change", function (event) {
            //       cartItems();
            showPrice();
            console.log("shippment is selected");
            $("#smspopup").hide();
                                        ...
                                        <div class="form-group mb10 clearfix">
                                            <div class="floating-form">
                                                ...
                                                <input style="display:none" type="password" name="fakepasswordremembered" />--%>
                                                    <asp:TextBox runat="server" ID="letstarttxtemail" Text="" class="floating-input" MaxLength="55" placeholder=" " autocomplete="auto-txtemail" TextMode="search" onkeyup="javascript:keyboardup(this);" onblur="javascript:onBlurValidate(this);"></asp:TextBox>
                                    ...
                                    <div class="form-group row p15 text-left clearfix">
                                        <div class="col-sm-20 pv15 br_dark">
                                            <div id="divshopcounter" runat="server">
                                                ...
                                                <p class="mb0">
                                                    Pickup from bigtop electronic warehouse
                                          <br />
                                                    84-90, Paramtta Road,
 ...
 <div class="row" runat="server" id="dropin">
  ...
  <%--  <script type="text/javascript">
    ...
    <!-- Event snippet for PayNow-Button-Tracking conversion page In your html page, add the snippet and call gtag_report_conversion when someone clicks on the chosen link or button. --> <script> function gtag_report_conversion(url) { var callback = function () { if (typeof(url) != 'undefined') { window.location = url; } }; gtag('event', 'conversion', { 'send_to': 'GTM-5R78S54/-bTxCKf5stYBEO65r_UD', 'transaction_id': '', 'event_callback': callback }); return false; } </script>
      ...
      <%--  <script src="https://js.braintreegateway.com/web/3.47.0/js/client.min.js" type="text/javascript"></script>--%>

        <script src="https://js.braintreegateway.com/web/dropin/1.22.0/js/dropin.min.js" type="text/javascript"></script>
       <script src="https://js.braintreegateway.com/web/3.47.0/js/three-d-secure.min.js" type="text/javascript"></script>
...
<script src="https://www.paypalobjects.com/api/checkout.js" data-version-4 log-level="warn" type="text/javascript"></script>
<script src="https://js.braintreegateway.com/web/3.59.0/js/client.min.js" type="text/javascript"></script>
    <script src="https://js.braintreegateway.com/web/3.59.0/js/data-collector.min.js" type="text/javascript"></script>
<script src="https://js.braintreegateway.com/web/3.59.0/js/paypal-checkout.min.js" type="text/javascript"></script>

     <script type="text/javascript">
           var dropin;

         //function braintreeIntegration () {
         $(document).ready(function () {
             ...
             var a = $("#<%=totalAmount.ClientID%>").text();
            // var a = "10.00";
             //console.log(a);
...
braintree.client.create({
  ...
  // Set up other Braintree components
       ...
       <script type="text/javascript" language="javascript">
         ...
         var amt1 = $("#<%=totalAmount.ClientID%>").text();
         console.log(amt1);
       //  alert(totamt.innerHTML.replace('<b>','').replace('</b>','').replace('$',''));
         ...
         $.ajax({
             type: "POST",
             url: "Express_Checkout.aspx/SaleTrans_Paypal",
             ...
             success: function (data) {
                 if (data.d.includes("Error") == false) {
                    ...
                    document.getElementById('navcart').style.display = "none";
                     ...
                     alert("Order Amount Missmatch.Please Checkout Again")
                     window.location.href = "Home.aspx";
                 }
...
//alert(err)
       ...
       //  alert(totamt.innerHTML.replace('<b>','').replace('</b>','').replace('$',''));
         ...
         $.ajax({
             type: "POST",
             url: "Express_Checkout.aspx/SaleTrans",
