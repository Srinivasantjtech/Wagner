    ...
    protected void Application_BeginRequest(object sender, EventArgs e)
    {
        ...
        if (strRawUrl.Contains("/pd/"))
        {
          ...
          //  objErrorhandler.CreateLog("inside    global" + DateTime.Now.ToString("hh.mm.ss.ffffff") + HttpContext.Current.Request.RawUrl.ToString());
             ...
             if (HttpContext.Current.Request.Url.ToString().Contains("https:") == true)
        {
            Response.RedirectPermanent(HttpContext.Current.Request.Url.ToString().Replace("https:", "http:"));
        ...
        if (strRawUrl.Contains("/mpd/"))
        {
            ...
            else
            {
                ...
                try
                {
                    url = "/" + mainstrrawurl[mainstrrawurl.Length - 2] + "/ps/";
        ...
        if (HttpContext.Current.Request.UrlReferrer == null || HttpContext.Current.Request.RawUrl.ToString().Contains(".pdf") == true)
        {
            ...
            // string urlref = HttpContext.Current.Request.UrlReferrer.ToString();
         ...
         (strRawUrl.Contains("/electronics-news/")) ||
           (strRawUrl == "/") ||
              (strRawUrl.ToLower().Contains("/404new.htm")) ||
               ...
               (strRawUrl.ToLower().Contains("/ourbrands/")) ||
              (strRawUrl.ToLower().Contains("/brandstore/")) 
      )
    )
            {
                Response.RedirectPermanent("/404new.htm");
           ...
           (strRawUrl.EndsWith("/pl/")) ||
                 ...
                 ))
            {
                    ...
                    !(strUrl.Contains("404new.htm")))
                {
                    ...
                    string lurl = HttpContext.Current.Request.RawUrl.ToString();
                    if ((lurl.Contains(".pdf") && !lurl.ToLower().Contains("attachments/attachments")))
                    {
                        lurl = lurl.Replace("/Attachments/", "/Attachments/attachments/").Replace("/attachments/", "/Attachments/attachments/");
      ...
      TradingBell.WebCat.Helpers.Security objSecurity = new TradingBell.WebCat.Helpers.Security();
            ...
            )
        {
            if (ConfigurationManager.AppSettings["REQUIRE_LOGIN"].ToString().Equals("YES"))
            {
                ...
                else if ((strUrl.Contains("attachments")) && !(strUrl.Contains("cataloguedownload.aspx")))
                {
