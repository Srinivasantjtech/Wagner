...
public partial class UC_loginMS : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (txtUserName.Text != string.Empty)
            {
                ...
                if (txtUserName.Text.Substring(0, 1) == ",")
                {
                    string[] strusername = txtUserName.Text.Split(',');

                    txtUserName.Text = strusername[strusername.Length - 1];
                }
            ...
            if (txtPassword.Text != string.Empty)
            {
                ...
                if (txtPassword.Text.Substring(0, 1) == ",")
                {
                    string[] strpwd = txtPassword.Text.Split(',');

                    txtPassword.Text = strpwd[strpwd.Length - 1];
                }
                else if (txtPassword.Text.Contains(",") == true)
                {
                                    ...
                                    if (Role != null)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                else if (HttpContext.Current.Session["SUPPLIER_NAME"] != null)
                                                {

                                                    string str = objHelperServices.SimpleURL_MS_Str(HttpContext.Current.Session["SUPPLIER_NAME"].ToString(), "mct.aspx", false);
                                                    Response.Redirect(str + "/mct/", false);
                                                }
                                                else
                                                {
                                                    Response.Redirect("/vip-vision/919/mct/", false);
