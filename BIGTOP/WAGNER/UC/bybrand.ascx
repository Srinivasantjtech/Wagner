...
<script type="text/javascript">
    ...
    function geturl() {
        ...
        if (e.options[e.selectedIndex].text != 'Select Category') {
            ...
            var url = window.location.href.replace(rootPath, '');

            //window.location.href = fullPath;
            $.ajax({
                type: "POST",
                url: "/GblWebMethods.aspx/setsession_brand_dd",
                data: "{'urlpath':'" + strURL + "','strpath':'" + strURL1 + "'}",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data) {
                    //alert("/ps=" + ddlattrvalue);
                    //window.location = "/ps=" + ddlattrvalue;
                    // document.forms[0].submit();
