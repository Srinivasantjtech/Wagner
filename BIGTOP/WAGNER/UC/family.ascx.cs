 ...
 string iRecordsPerPage;
    ...
    private void ConstructFamilyData(string familyid, DataRow[] SourceFtb, DataSet tempDs, int tableinx )
    {
        try
        {
            ...
            if (tempDs != null && tempDs.Tables.Count > 0 && tempDs.Tables[tableinx].Rows.Count > 0)
            {
                ...
                objErrorHandler.CreateLog("Ds length"+ Ds.Tables[familyid].Rows.Count);
    ...
    public string ST_FamilypageALLData()
    {
     ...
     // _stmpl_container.SetAttribute("ST_Category",ST_Categories());
            ...
            objErrorHandler.ExeTimelog = "family Page loading time = " + stpw.Elapsed.TotalSeconds.ToString();
            ...
            if (EADs != null)
            {
                if (EADs.Tables["FamilyPro"].Rows.Count > 0)
                {
                 ...
                 //   string sortOrder = "SORT_ORDER";
                    ...
                    //DrMain = EADs.Tables["FamilyPro"].Select("FAMILY_ID='" + _Fid + "'", "SORT_ORDER");
                    objErrorHandler.CreateLog("DrMain length"+ DrMain.Length) ;
                    if (DrMain.Length > 0)
                    {
                        // Famtb = Dr.CopyToDataTable();
                        objErrorHandler.CreateLog("ConstructFamilyData");
    ...
    public string ST_Familypage(string fid, string Rawurl)
    {
        ...
        try
        {
            ...
            if (_Fid != null)
            {
                //if(Ds != null && Ds.Tables.Count>0 && Ds.Tables[_Fid]!= null && Ds.Tables[_Fid].Columns.Count>0)
                //    hffamcolumncnt.Value = Ds.Tables[_Fid].Columns.Count.ToString();
                ...
                //by jtech
                //stopwatch.Start();
                CScontentvalue = ObjFamilyPage.GenerateHorizontalHTMLJson(_Fid, Ds, dsPriceTableAll, EADs,Rawurl, _Fid);
                //stopwatch.Stop();
              ...
              //  objErrorHandler.CreateLog("GenerateHorizontalHTMLJson:" + "=" + stopwatch.Elapsed);
                ...
                else if (SFamtb != null)
                {
                    ...
                    foreach (DataRow DR in SFamtb.Rows)
                    {
                        ...
                        //stopwatch.Start();
                        cssubfamilycontent = ObjFamilyPage.GenerateHorizontalHTMLJson(DR["Family_id"].ToString(), Ds, dsPriceTableAll, EADs,Rawurl, _Fid);
                        //stopwatch.Stop();
    ...
    //    return prodtable;
    //}
    //string fid, string url, string list, string value, string screen
    public string sortProduct(string fid, string url, IList<DataClass> array , string screen)
    {
        EADs = (DataSet)HttpContext.Current.Session["FamilyProduct"];
        _Fid = fid;
        if (HttpContext.Current.Request.QueryString["fid"] != null)
        ...
        //int rtn = GetFamilyAllData(_Fid);
        return ObjFamilyPage.SortProductsList(_Fid, Ds, dsPriceTableAll, EADs, url, array, screen);
    }  
   ...
   public DataSet ConvertXMLToDataSet(string xmlData)
    ...
    //public static Object SortProduct(string list, string value)
    //{
