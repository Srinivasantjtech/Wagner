...
<script language="javascript" type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var acc = document.getElementsByClassName("tbl_filter_btn");
        var i;
        ...
        $('div.dropdown-menu.tbl_filter a').on('click', function (event) {
...
<script type="text/javascript">
   ...
   <script type="text/javascript">
       ...
       $(document).ready(function () {
           if ($("#" + '<%= hffamcolumncnt.ClientID %>').val() > 8) {
               $("#res_table").hide();
               $("#res_div").show();
           }
       });
       ...
       function decrement(id) {
        ...
        function clearfilter()
        {
             location.reload();
//            console.log(str);
//            $.each($('select[multiple].web.filter_drp'), function (key, selectbox) {
//             var ids = selectbox.id;
...
//            });
            ...
            //$(function () {
            //     var values = [];
            //    $('select[multiple].web.filter_drp').on('change', function () {
            ...
            //             var id = $(this).attr('id');
            //             $("#"+id).hide();
            //             values = values.filter(a => a.option !== title)
            //             $selectedOptions.each(function () {
            //                 var val = { "option": $(this).parent().attr('title'), "value": $(this).text() };
            //                values.push({ "option": $(this).parent().attr('title'), "value": $(this).text() });
            //             });
            //             console.log(values);
            //             sortproduct(values,"web");
            // });
         ...
         $(document).ready(function() {
          ...
          //  onChange: function() {
          //    console.log($('#multipleSelect2').val());
          //  }
          //   });
             var values = [];
             var values_mob = [];

             $('#innertable').on('change', 'select[multiple]', function() {
                 //alert('here123');
                 ...
                 values = values.filter(a => a.option !== title)
                $selectedOptions.each(function () {
                     var val = { "option": $(this).parent().attr('title'), "value": $(this).val() };
                     values.push({ "option": $(this).parent().attr('title'), "value": $(this).val() });
                 });
                 console.log(values);
                 sortproduct(values, "web");
            });
             ...
             //    //    //}
             //    //     console.log(title);
             //    //     console.log(values.findIndex(v => v.option === title));
             //    ////values.splice(values.findIndex(v => v.option === title),1 );
             //    //});
             //    //values = arrayRemove(values, 6);
             ...
             //});
             // console.log("mob dropdown");


           
             //$('select[multiple]').on('change', function () { alert('helo'); });

             //$(".tbl_filter_wrap").on('change', 'select', function () { alert('helo123'); });

          

            //$('select[multiple].mob.filter_drp').on('change', function() {
            // console.log("mob dropdown clicked");
            //     var $selectedOptions = $(this).find('option:selected');
            //     var title = $(this).attr('title');
            //     values_mob = values_mob.filter(a => a.option !== title)
            //     $selectedOptions.each(function () {
            //        values_mob.push({ "option": $(this).parent().attr('title'), "value": $(this).text() });
            //     });
            //     console.log(values_mob);
            //     sortproduct(values_mob,"mob");
            //});
             ...
             $('.ms-options-wrap>.ms-options>a').on('click', function (event) {
                //$(this).parent().removeClass('show');
                 console.log("clicked123")
                 $(this).siblings(".ms-search").child().val() = '';
                 console.log($(this).siblings(".ms-search"));
                 console.log($(this).siblings(".ms-search").children("input").val());
             });
             $('.ms-options-wrap>.ms-options>ul').on('click', function (event) {
                //$(this).parent().removeClass('show');
                console.log("clicked");
                $(this).parent().parent().removeClass('ms-active');
            });
...
<script type="text/javascript">
    ...
    function sortproduct(array, screen) {

        //alert(list);
        // alert(array);
        //  console.log(array);
        var fid = $("#" + '<%= hffid.ClientID %>').val();
        var url = $("#" + '<%= hfrawurl.ClientID %>').val();
       // var screen = "web";
       
        var productSort = {};
        productSort.fid = fid;
        productSort.url = url;
        productSort.sortoptions = array;
        productSort.screen = screen;
      
        $.ajax({
            type: "POST",
            url: "/fl.aspx/SortProduct",
            data: JSON.stringify({ productSort: productSort }),
            contentType: "application/json;",
            dataType: "json",
            success: OnSortingSuccess,
            error: OnSortingFailure
        });

    }


    function OnSortingSuccess(result) {
         console.log(result.d);
        if (result.d != null && result.d != "-1" && result.d != "") {
            var text = result.d;
            var q;
            //if (text.includes('fmly_mbPro_detail') || text.includes('mbPro_detail_empty')) {
            //    // Found world
            //  //  alert("asdf");
            //    var q = document.getElementById("fmly_mbPro_wrap");
            //    if (text.includes('mbPro_detail_empty')) {
            //        $("#fmly_mbPro_wrap").removeClass("masonry");
            //    }
            //    else {
            //        $("#fmly_mbPro_wrap").addClass("masonry");
            //    }
            //}
            //else {
            //    var q = document.getElementById("RelateedPge_wrap");
            //}
           // q.innerHTML = "";
            // var q = document.getElementById("innertable");
            //q.innerHTML = result.d;
            $("#innertable").html(result.d);
             //console.log(result.d);
            ...
            $.each($('select[multiple]'), function (key, selectbox) {
                ...
                //$('#' + selectbox.id).find('option').each(function (idx, option) {
                ...
                $('#'+selectbox.id).multiselect({
                    search: true,
                     placeholder: $('#'+selectbox.id).attr('title'),
                    searchOptions: {
                        'default': $('#'+selectbox.id).attr('title')
                    },
                    selectAll: true
                });
    ...
    function OnSortingFailure(result) {
        alert("Unable to sort product ");
    }
</script>
...
<%=ST_FamilypageALLData()%>
  ...
  <input type="hidden" id="Hidden1" name="hfclickedattr" value="" />
         ...
         <asp:HiddenField ID="hffamcolumncnt" runat="server"/>
<div id="testdiv" class="ppdivtest" ></div>
<div id="testdivimg" class="testdivimgcss" ></div>

<%--<script type="text/javascript">
    $(function () {
           $('.tbl_filter_wrap .fmly_filter ').on('change', 'select[multiple]', function () {
                 alert('here');
             });
