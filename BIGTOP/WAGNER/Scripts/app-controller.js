...
//class AppController {
  //constructor() {
    ...
    //this.endpoint= $("#" + '<%= HiddenField2.ClientID %>').val();
  ...
  // getcurrentsub();
   ...
   // getcurrentsub();
   // };
  //  alert(  this._currentSubscription);
  ...
  //}
    ...
    //    alert(subscription);
    //}
  //    var markers =this._currentSubscription;
 ...
 function getWebPushInfo( payload, vapidHeaders) {
    ...
    var pendpoints=  this.endpoint.trim();
  
    console.log(pendpoints);
 ...
 //        swRegistration.pushManager.getSubscription()
 //  .then(function(subscription) {
 //      this._myendpoint=subscription.endpoint;
 //      this._currentSubscription=subscription;
 //   console.log(this._myendpoint);
 ...
 //}
 function sendPushMessage(myendpoint,subscription, payloadText,endpoint) {
     // Let's look at payload
   
     //if(myendpoint==null)
     //{
     //    this._myendpoint= getcurrentsub();
     
     //}
     //else
     //{
        this._myendpoint=myendpoint;
     //}
   ...
   //  this._currentSubscription=getcurrentsub();
    // console.log(_currentSubscription);
   //  this.ready = Promise.resolve();
     this.endpoint=endpoint;
     getvapidpromise(endpoint);
     let payloadPromise = Promise.resolve(null);
     console.log(subscription);
    if (payloadText && payloadText.trim().length > 0) {
      payloadPromise = EncryptionHelperFactory.generateHelper()
      .then((encryptionHelper) => {
         ...
         JSON.parse(subscription), payloadText);
        // JSON.parse('{"endpoint":"https://fcm.googleapis.com/fcm/send/ckcBUZGNiBw:APA91bGA1gBzIwJy1YcRJd-l9sc…RVXAEhWA9t-ujLK9qmc8RVmku3C1gb2qM9r0ksHxRBkP6sBF208_FnEpleWndLwZdQLDW5UhmY","keys":{"p256dh":"BK8UFdEKLgMF7ZLZokE0X7qn5_lMeohqm8rDQskP8aQjcujNyjB3ws8OHvE5F6yeoIy5C-cLZgx-1zebwuRTI1I=","auth":"eMcSlAHDq55ekbVEixs4TQ=="}}  '), payloadText);
      });
    }

// Vapid support
 function getvapidpromise(_myendpoint)
 {
    
 this.vapidPromise = EncryptionHelperFactory.createVapidAuthHeader(
      this._applicationKeys,
    _myendpoint,
      'mailto:simple-push-demo@gauntface.co.uk');
 }
 ...
 infoFunction = () => {
     return this.getWebPushInfo( payload,
      vapidHeaders);
 };


      if (this._myendpoint.indexOf(
        'https://android.googleapis.com/gcm/send') === 0) {
          infoFunction = () => {

              navigator.serviceWorker.register('service-worker-wagner.js')
    .then(function(swReg) {
        console.log('Service Worker is registered', swReg);
