  ...
  //  string _fid = "";
    ...
    private void GetPageConfig()
    {
        try
        {
            ...
            {
                if ((Request.Url.ToString().ToLower().Contains("bb.aspx")))
                {
    ...
    {
        _stmpl_pages.SetAttribute("CDN", System.Configuration.ConfigurationManager.AppSettings["CDN"].ToString());
    ...
    //    //    return "";
    //    //}
    //    if (Request["srctext"] != null && Request["srctext"].ToString() == string.Empty)
    //    {
    //        return "CLEAR";
    //    }
    ...
    //        if (dsfamprod.Tables != null && dsfamprod.Tables.Count > 0 && Request.QueryString["fid"] != null && Request.QueryString["fid"].ToString() != string.Empty && Request.Url.ToString().ToLower().Contains("byproduct.aspx"))
    //        {
    ...
    //                    objHelperServices.SimpleURL(_stmpl_records, newurl, "fl.aspx");
    ...
    //                    objHelperServices.SimpleURL(_stmpl_records, newurl, "pd.aspx");
    ...
    //                        string href = "ps.aspx?&id=0&searchstr=" + HttpUtility.UrlEncode(_drow["SearchString"].ToString()) + "&type=" + dscategory.Tables[0].TableName.ToString() + "&bname=" + HttpUtility.UrlEncode(_bname.ToString()) + "&value=" + HttpUtility.UrlEncode(_drow["Category_Name"].ToString()) + "&byp=2&Path=" + eapathsubcat;
    ...
    //            {
    //                bool rUrl = false;
    //                if ((Request.Url.ToString().ToLower().Contains("bb.aspx")) || (Request.Url.ToString().ToLower().Contains("byproduct.aspx")))
    //                {
    //                    rUrl = true;
    //                }
    //                string sl2 = "";
   ...
   //   string strFile = System.Configuration.ConfigurationManager.AppSettings["ProdimgSepdomainUrl"].ToString();
       ...
       // bool catdispflag = false;
        ...
        try
        {
            ...
            if (dscat == null)
            {
                ...
                string rawurl = Request.RawUrl.ToLower().ToString();
                if (rawurl.Contains("xx"))
                {


                    string querystring = objHelperServices.URlStringReverse(rawurl);
                    string rurl = querystring;

                    string[] rurl1 = Regex.Split(rurl, "/");
                    if (rurl1.Length > 1)
                    {
                        rurl = rurl1[1].Replace("~~", "/");
                    }
                    string strvalue = objHelperServices.SimpleURL_Str( rurl, "ps.aspx",true);
                    strvalue = strvalue + "/ps/";
                    Response.RedirectPermanent(strvalue,false); 
                }
            ...
            if (dsfamprod.Tables != null && dsfamprod.Tables.Count > 0 && Request.QueryString["fid"] != null && Request.QueryString["fid"].ToString() != string.Empty && requrl.Contains("byproduct.aspx"))
            {
                iTotalProducts = dsfamprod.Tables[0].Rows.Count;
                if (iRecordsPerPage >= iTotalProducts)
                {
    ...
    //        DataSet dsPriceTableAll = new DataSet();
           // string tmpProds = string.Empty;
            ...
            if (dscat.Tables["FamilyPro"].Rows.Count  > 0)
            {
                ...
                foreach (DataRow drpid in dscat.Tables["FamilyPro"].Rows)
                {
                    ...
                    //_stmpl_records.SetAttribute("PRODUCT_CODE", drpid["PRODUCT_CODE"]);
                    //int _pid;
                    //if (drpid["PRODUCT_ID"].ToString() != null && drpid["PRODUCT_ID"].ToString() != string.Empty)
                    //{
       ...
       //             string PriceTable = string.Empty; 
                    ...
                    string stk_sta_desc = "";
                    stk_sta_desc = drpid["STOCK_STATUS_DESC"].ToString().Trim().ToUpper();
                    ...
                    else
                    {
                        ...
                        if ((stk_sta_desc == "DISCONTINUED NO LONGER AVAILABLE" || stk_sta_desc == "TEMPORARY UNAVAILABLE NO ETA" || stk_sta_desc == "TEMPORARY_UNAVAILABLE NO ETA"))
                        {
                            ...
                            if (stk_sta_desc == "DISCONTINUED NO LONGER AVAILABLE")
                            {
                                ...
                                //objErrorHandler.CreateLog("_SearchString--" + _SearchString);
                                //objErrorHandler.CreateLog("PRODUCT_CODE--" + drpid["PRODUCT_CODE"].ToString().ToUpper());
                ...
                //                    bool samecodenotFound = (bool)rtntbl.Rows[0]["samecodenotFound"];
                 ...
                 //                       string strurl = rtntbl.Rows[0]["ea_path"].ToString();
                    ...
                    _stmpl_records.SetAttribute("ATTRIBUTE_ID_5", drpid["PRODUCT_PRICE"]);
                    ...
                    if (drpid["CATEGORY_PATH"].ToString() != null)
                    {
                        ...
                        //if ( drpid["CATEGORY_PATH"].ToString().ToUpper().Contains("SUPPLIER PRODUCT FEED"))
                        //{
                        //    objErrorHandler.CreateLog(categorypath + drpid["FAMILY_NAME"]);
                        //    BindToST = false;
                        //}
                    ...
                    if (Convert.ToInt32(drpid["PRODUCT_COUNT"].ToString()) > 1)
                    {
                        ...
                        objHelperServices.SimpleURL(_stmpl_records, WAG_Root_Path + "////" + drpid["FAMILY_ID"] + "////" + categorypath + "////" +
                                          drpid["FAMILY_NAME"], "fl.aspx");
                    }
                    else
                    {
                        _stmpl_records.SetAttribute("TBT_SUB_FAMILY", false);
                        ...
                        objHelperServices.SimpleURL(_stmpl_records, WAG_Root_Path + "////" + drpid["FAMILY_ID"] + "////" + drpid["PRODUCT_ID"] + "=" + drpid["PRODUCT_CODE"] + "////" +
                                                 categorypath + "////" +
                                                                drpid["FAMILY_NAME"], "pd.aspx");
                    ...
                    //else
                    //{
                    //    Fil = new System.IO.FileInfo(strFile + drpid["Prod_Thumbnail"].ToString().Replace("/", "\\"));
                    //    if (Fil.Exists)
                    //        _stmpl_records.SetAttribute("ATTRIBUTE_ID_452", drpid["Prod_Thumbnail"]);
                    //    else
                    //        _stmpl_records.SetAttribute("ATTRIBUTE_ID_452", "/images/noimage.gif");
                    ...
                    //}
                     ...
                     if (drpid["Prod_Thumbnail"].ToString().Contains("noimage.gif"))
                    {
                        _stmpl_records.SetAttribute("ATTRIBUTE_ID_452", strprodimg_sepdomain+"/images/noimage.gif");
                    }
                    else
                    {
                        //Fil = new System.IO.FileInfo(strFile + drpid["Prod_Thumbnail"].ToString().Replace("/", "\\"));
                        //if (Fil.Exists)
                        //    _stmpl_records.SetAttribute("ATTRIBUTE_ID_452", strprodimg_sepdomain+drpid["Prod_Thumbnail"]);
                        //else
                        //    _stmpl_records.SetAttribute("ATTRIBUTE_ID_452", strprodimg_sepdomain+"/images/noimage.gif");
                        _stmpl_records.SetAttribute("ATTRIBUTE_ID_452", strprodimg_sepdomain + drpid["Prod_Thumbnail"]);
                    }
        ...
        //objErrorHandler.CreateLog(lstrows.Length.ToString() + "lstrow lenth" + isproductavailable);
        //        }
        //        catch (Exception ex)
        //        {
        ...
        //        }
                ...
                //UC_searchctrl uc = new UC_searchctrl();
                //uc.urlping = "ps.aspx?" + HttpContext.Current.Request.QueryString.ToString();
       //         _stmpl_container.SetAttribute("TBT_REWRITEURL_REL", "ps.aspx?" + HttpContext.Current.Request.QueryString.ToString());
     //           _stmpl_container.SetAttribute("TBW_PRODUCT_COUNT", iTotalProducts);
     //           _stmpl_container.SetAttribute("TBW_CATEGORY_NAME", category_nameh.ToUpper());
                _stmpl_container.SetAttribute("TBWDataList", lstrows);
               ...
               // int icolstart11 = 0;
                ...
                if (dscategory != null && dscategory.Tables.Count > 0)
                {
                    ...
                    if (_DCRow != null && _DCRow.Length > 0)
                    {
                        ...
                        foreach (DataRow _drow in _DCRow)
                        {
                            ...
                            _stg_records_container = new StringTemplateGroup("searchrsltmultilistcontainer", stemplatepath);
                            _stmpl_records_tmpl = _stg_records.GetInstanceOf("searchrsltproducts" + "\\" + "multilistitem");
                            if (_drow["CATEGORY_ID"].ToString().Contains("SPF-") == false)
                            {
                                _stmpl_records_tmpl.SetAttribute("TBW_LIST_VAL", _drow["Category_Name"].ToString());
                                _stmpl_records_tmpl.SetAttribute("TBW_SELECTED_ID", "1");
                                ...
                                if (breadcrumb_ct.Tables.Count > 0 && breadcrumb_ct.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    else
                                    {
                                        if (breadcrumb_ct.Tables[0].Rows.Count > 0 && breadcrumb_ct.Tables[0].Rows.Count > 2)
                                        {
                                            string catvalue = breadcrumb_ct.Tables[0].Rows[2]["ItemValue"].ToString();
                                            if (catvalue.ToLower() == _drow["Category_Name"].ToString().ToLower())
                                            {
                                                _stmpl_records_tmpl.SetAttribute("TBW_SELECTED", "selected=\"selected\" ");
                                            }
                            ...
                            //  string href = "powersearch.aspx?&id=0&searchstr=" + HttpUtility.UrlEncode(_drow["SearchString"].ToString()) + "&type=" + dscategory.Tables[0].TableName.ToString() + "&bname=" + HttpUtility.UrlEncode(_bname.ToString()) + "&value=" + HttpUtility.UrlEncode(_drow["Category_Name"].ToString()) + "&byp=2&Path=" + eapathfil;
                            string href = "ps.aspx?&amp;id=0&amp;searchstr=" + HttpUtility.UrlEncode(_drow["SearchString"].ToString()) + "&amp;type=" + dscategory.Tables[0].TableName.ToString() + "&amp;bname=" + HttpUtility.UrlEncode(_bname.ToString()) + "&amp;value=" + HttpUtility.UrlEncode(_drow["Category_Name"].ToString()) + "&amp;byp=2&amp;Path=" + eapathfil;
                           // _stmpl_records_tmpl.SetAttribute("TBW_LIST_VAL1", _CategorySelect + "@@category@@" + _drow["Category_Name"].ToString());
                           _stmpl_records_tmpl.SetAttribute("TBW_LIST_VAL1", href);
                ...
                if (dssubcategory != null && dssubcategory.Tables.Count > 0 && dssubcategory.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (_DCRow1 != null && _DCRow1.Length > 0)
                    {
                        ...
                        foreach (DataRow _drow in _DCRow1)
                        {
                            ...
                            if (maincat != string.Empty)
                            {
                                maincat = "////" + maincat;
                            }
                       //     _stmpl_records_tmpl1.SetAttribute("TBW_LIST_VAL1", _CategorySelect + maincat + "@@category@@" + _drow["Category_Name"].ToString());
                ...
                //try
                //{
                //    if (Request.QueryString["srctext"].ToString() != string.Empty)
                //    {
                //        nextstring = "&srctext=" + Request.QueryString["srctext"].ToString().Replace("\"", "%22").Replace("&", "%26").Replace("#", "%23");
...
sHTML = _stmpl_container.ToString().Replace("<option value=\"" + iRecordsPerPage.ToString() + "\">", "<option value=\"" + iRecordsPerPage.ToString() + "\" selected =selected>");
           ...
           // string BCEAPath = _BCEAPath.Replace("'", "###");
            ...
            {
                htmlpsirecords.Value = System.Configuration.ConfigurationManager.AppSettings["iRecordsPerPage"].ToString();
                HttpContext.Current.Session["RECORDS_PER_PAGE_POWERSEARCH"] = htmlpsirecords.Value;
            }
        }
        catch (Exception ex)
        {
  ...
  //      return objHelperServices.StripWhitespace(sHTML);
    ...
    //            string subrealBCEAPath = string.Empty;
    ...
    //            if (Tosuit_ModelImage != string.Empty)
    //            {
    ...
    //                string sl3 = string.Empty;
    //                string tosuite_brand = string.Empty;
    //                string tosuite_model = string.Empty;
    //                if (rUrl)
    //                {
    //                    if (Request.QueryString["cid"] != null && Request.QueryString["cid"].ToString() != string.Empty)
    //                        sl1 = Request.QueryString["cid"].ToString();
    //                    if (Request.QueryString["sl1"] != null && Request.QueryString["sl1"].ToString() != string.Empty)
    //                        sl2 = Request.QueryString["sl1"].ToString();
    //                    if (Request.QueryString["sl2"] != null && Request.QueryString["sl2"].ToString() != string.Empty)
    //                        sl3 = Request.QueryString["sl2"].ToString();
    //                    if (Request.QueryString["tsb"] != null && Request.QueryString["tsb"].ToString() != string.Empty)
    //                        tosuite_brand = Server.UrlEncode(Request.QueryString["tsb"].ToString());
    //                    if (Request.QueryString["tsm"] != null && Request.QueryString["tsm"].ToString() != string.Empty)
    ...
    protected string ST_BrandAndModelProductListNewJson()
    {
        string _tempfid = string.Empty;
   //     string category_nameh = string.Empty;
        string strFile = System.Configuration.ConfigurationManager.AppSettings["CDNROOT"].ToString();
        System.IO.FileInfo Fil = null;
        bool BindToST = true;
        //if (Request["srctext"] != null && Request["srctext"].ToString() == string.Empty)
        //{
       ...
       //         bool IsEcomEnabled = objHelperServices.GetIsEcomEnabled(userid);
        ...
        //        string CustomerType = objUserServices.GetCustomerType(Convert.ToInt32(userid));
          ...
          //          _stmpl_records.SetAttribute("TBT_ECOMENABLED", IsEcomEnabled);
                    ...
                    //_stmpl_records.SetAttribute("MIN_ORD_QTY", oProd.GetMinOrdQty(Convert.ToInt32(drpid["PRODUCT_ID"].ToString())).ToString());
           //         _stmpl_records.SetAttribute("MIN_ORD_QTY", drpid["MIN_ORD_QTY"]);
                    trmpstr = drpid["FAMILY_NAME"].ToString().Replace("<ars>g</ars>", "&rarr;").Replace("<sps>l</sps>", "<font face=\"Symbol\">l</font>");
                    if (_ViewType == "GV")
                    {
          ...
          //              _stmpl_records.SetAttribute("TBT_BUY_PRODUCT", true);
                  ...
                  //  {
                    ...
                    else
                    {
                        ...
                        desc = desc + " " + drpid["Family_ShortDescription"].ToString().Replace("\r", "").Replace("\r\n", "").Replace("\n", "").Replace("<ars>g</ars>", "<font face=\"Wingdings 3\">g</font>").Replace("<sps>l</sps>", "<font face=\"Symbol\">l</font>").Replace("<sym>r</sym>", "<font face=\"WP MathA\">r</font>").Replace("<sym>s</sym>", "<font face=\"WP MathA\">s</font>");
                        ...
                        if (desc.Length > 120)
                        {
                            ...
                            _stmpl_records.SetAttribute("ATTRIBUTEDESC", desc + "...");
                        }
                        else
                        {
                            _stmpl_records.SetAttribute("ATTRIBUTEDESC", desc);
                ...
                if (HttpContext.Current.Session["SortOrder"] != null && HttpContext.Current.Session["SortOrder"] != "")
                {
                    ...
                    if (HttpContext.Current.Session["SortOrder"] != null && HttpContext.Current.Session["SortOrder"] == "Latest")
                        ...
                        _stmpl_container.SetAttribute("SortBy", "Price High To Low");
                    else if (HttpContext.Current.Session["SortOrder"] != null && HttpContext.Current.Session["SortOrder"] == "popularity")
                        _stmpl_container.SetAttribute("SortBy", "Popular");
                    else
                        _stmpl_container.SetAttribute("SortBy", "Latest");
                ...
                _stmpl_container.SetAttribute("TBT_REWRITEURL_REL", "bb.aspx?" + HttpContext.Current.Request.QueryString.ToString());
                _stmpl_container.SetAttribute("TBWDataList", lstrows);
                _stmpl_container.SetAttribute("CDN", System.Configuration.ConfigurationManager.AppSettings["CDN"].ToString());
                ...
                {
                    DataTable dsCat = (DataTable)((DataSet)HttpContext.Current.Session["Category_Attributes"]).Tables["Models"];
                    if (dsCat != null && dsCat.Rows.Count != 0)
                    {
                        DataRow[] drs = dsCat.Select("eapath like '%" + _tsm + "' and brandvalue='" + _tsb +"'"  );
                        if (drs.Length > 0)
                        {
                            string[] model = drs[0]["eapath"].ToString().Split(':');
                            if (model.Length == 2)
                            {
                                _stmpl_container.SetAttribute("TBT_TOSUITE_BRAND", model[0]);
                                _stmpl_container.SetAttribute("TBT_TOSUITE_MODEL", model[1]);
                            }
                            else
                            {
                                _stmpl_container.SetAttribute("TBT_TOSUITE_BRAND", _tsb);
           ...
           //                     _stmpl_records_tmpl1.SetAttribute("TBW_SELECTED_ID", "1");
                                ...
                                //string href = string.Empty;
                                //if (subcatvalue == string.Empty)
                                //    href = _realBCEAPath + "////" + "Category=" + _drow["Category_Name"];
                                //else
                                //{
                                //    //.Replace("+","||")
                                //    string _realBCEAPath1 = "////" + " " + "////" + " " + "////" + "CELLULAR ACCESSORIES" + "////" + _drow["brandvalue"].ToString().ToUpper() + "////" + HttpUtility.UrlEncode(_tsm.ToUpper()) + "////" + "Category=" + catvalue.ToUpper();
                ...
                if (dscatebybrand != null && dscatebybrand.Tables.Count>0   )
                {
                ...
                if (Tosuit_ModelImage != string.Empty)
                {
                  // Fil = new System.IO.FileInfo(strFile + Tosuit_ModelImage.Replace("/", "\\"));
                    bool ImageExits = objHelperServices.CheckImageExistCDN(strFile + Tosuit_ModelImage.Replace("/", "\\"));
        //            _stmpl_container.SetAttribute("CDNROOT", strFile);
   ...
   //             _stmpl_container.SetAttribute("TBW_PRODUCT_COUNT", iTotalProducts);
    ...
    //            _stmpl_container.SetAttribute("TBW_CATEGORY_NAME", category_nameh.ToUpper());
            ...
            else
            {
                ...
                HttpContext.Current.Session["RECORDS_PER_PAGE_BYBRAND"] = htmlirecords.Value;
  ...
  //      string WesCatalogId = System.Configuration.ConfigurationManager.AppSettings["WES_CATALOG_ID"].ToString();
      ...
      //      pricecode = objHelperDB.GetPriceCode(userid);
         ...
         //           string PriceTable = string.Empty;
                    ...
                    stk_sta_desc = drpid["STOCK_STATUS_DESC"].ToString().Trim().ToUpper();
                     //&& drpid["PROD_STOCK_FLAG"].ToString().Trim() == "-2"
                    //objErrorHandler.CreateLog(stk_sta_desc +" ps");
                    if (Convert.ToInt32(drpid["PRODUCT_COUNT"].ToString()) > 1)
                    {
                    }
                    else
             ...
             //                       bool samecodenotFound = (bool)rtntbl.Rows[0]["samecodenotFound"];
               ...
               //                         string strurl = rtntbl.Rows[0]["ea_path"].ToString();
                    ...
                    else
                    {
                        //System.IO.FileInfo Fil = new System.IO.FileInfo(strFile + drpid["Prod_Thumbnail"].ToString().Replace("/", "\\"));
                        //if (Fil.Exists)
                        //    _stmpl_records.SetAttribute("ATTRIBUTE_ID_452", strprodimg_sepdomain+drpid["Prod_Thumbnail"].ToString());
                        //else
                        //    _stmpl_records.SetAttribute("ATTRIBUTE_ID_452", strprodimg_sepdomain+"/images/noimage.gif");
                        _stmpl_records.SetAttribute("ATTRIBUTE_ID_452", strprodimg_sepdomain + drpid["Prod_Thumbnail"].ToString());
                    }
              ...
              //  }
                ...
                //sHTML = TBWDataListnew+  "<div class='clearfix text-center'></div>";

                sHTML = TBWDataListnew;
            }
        }
        catch (Exception ex)
        {
        ...
        try
        {
          ...
          //  int ictrows = 0;
           ...
           // string _byp = "2";
            ...
            else if ((HttpContext.Current.Request.Url.ToString().ToLower().Contains("ps.aspx")))
            {
                _searchstr = _value;
            }
            ...
            if ((HttpContext.Current.Request.Url.ToString().ToLower().Contains("bb.aspx")))
            {
               // int SubCatCount = 0;
                if (_type == null)
                {
            ...
            if ((HttpContext.Current.Request.Url.ToString().ToLower().Contains("ps.aspx")))
            {
                //if (HttpContext.Current.Session["RECORDS_PER_PAGE_POWERSEARCH"] != null)
    ...
    //        DataRow drpagect = dscat.Tables[0].Rows[0];
    //        DataRow drproductsct = dscat.Tables[1].Rows[0];
    //        if (dsfamprod.Tables != null && dsfamprod.Tables.Count > 0 && HttpContext.Current.Request.QueryString["fid"] != null && HttpContext.Current.Request.QueryString["fid"].ToString() != string.Empty && HttpContext.Current.Request.Url.ToString().ToLower().Contains("byproduct.aspx"))
    //        {
    //            iTotalProducts = dsfamprod.Tables[0].Rows.Count;
    //            if (iRecordsPerPage >= iTotalProducts)
    //            {
   ...
   //     string WesCatalogId = System.Configuration.ConfigurationManager.AppSettings["WES_CATALOG_ID"].ToString();
        ...
        //    return ""; 
        //}
        if (HttpContext.Current.Request["srctext"] != null && HttpContext.Current.Request["srctext"].ToString() == string.Empty)
        {
            return "CLEAR";
        }
        else if (HttpContext.Current.Request["srctext"] != null)
     ...
     //       int pricecode = 0;
          ...
          //          string PriceTable = "";
                    ...
                    stk_sta_desc = drpid["STOCK_STATUS_DESC"].ToString().Trim();
                 //&& drpid["PROD_STOCK_FLAG"].ToString().Trim() == "-2"
                   // objErrorHandler.CreateLog(stk_sta_desc + " bb");
                    BindToST = true;
